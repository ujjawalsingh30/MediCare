...
// When empty or incalid it return empty array ie:  []
...
// So this function take date-time slot strings and group them into a YYY-MM-DD with the time
...
export async function getServices(req, res) { 
    try {
        const list = await Service.find().sort({ createAt: -1 }).lean();
...
export async function getServiceById(req, res) {
    try {
        ...
        console.error("GetServicesById Error:", err);
        return res.status(500).json({
            success: false,
            message: "Server Error"
        });
